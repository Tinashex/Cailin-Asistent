...
let handler = async (m, { kyu, ctx, args, text, command, prefix }) => {
  ...
  try {
    const { buffer } = await ephoto('https://en.ephoto360.com/create-pixel-glitch-text-effect-online-769.html', ctx.query);
    await kyu.sendMessage(ctx.id, {
      image: buffer,
      caption: "✅ *PIXELGLITCH Text*\n📝 " + ctx.query
    }, { quoted: simpleQuoted(ctx) });
  } catch (e) {
    ctx.reply("❌ Gagal memproses efek *pixelglitch*: " + (e.message || e));
  }
};

handler.help        = ["pixelglitch"];
handler.tags        = ["ephoto"];
handler.command     = /^(pixelglitch)$/i;
handler.description = "pixelglitch text effect";
