...
let handler = async (m, { kyu, ctx, args, text, command, prefix }) => {
  ...
  try {
    const { buffer } = await ephoto('https://en.ephoto360.com/create-digital-glitch-text-effects-online-767.html', ctx.query);
    await kyu.sendMessage(ctx.id, {
      image: buffer,
      caption: "✅ *GLITCHTEXT Text*\n📝 " + ctx.query
    }, { quoted: simpleQuoted(ctx) });
  } catch (e) {
    ctx.reply("❌ Gagal memproses efek *glitchtext*: " + (e.message || e));
  }
};
...
handler.description = "glitchtext text effect";
