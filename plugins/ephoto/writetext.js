...
let handler = async (m, { kyu, ctx, args, text, command, prefix }) => {
  ...
  try {
    const { buffer } = await ephoto('https://en.ephoto360.com/write-text-on-wet-glass-online-589.html', ctx.query);
    await kyu.sendMessage(ctx.id, {
      image: buffer,
      caption: "✅ *WRITETEXT Text*\n📝 " + ctx.query
    }, { quoted: simpleQuoted(ctx) });
  } catch (e) {
    ctx.reply("❌ Gagal memproses efek *writetext*: " + (e.message || e));
  }
};
...
handler.description = "writetext text effect";
