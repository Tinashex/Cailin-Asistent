...
export async function loadAllSavedCloneSessions() {
  ...
  for (const num of clones) {
    if (num && num.length >= 9) {
      try {
        console.log(chalk.cyan(`[CLONE AUTO-LOAD] Memuat Sesi Clone Bot: +${num}`));
  ...
  try {
    ...
    cloneSock.ev.on('connection.update', async (update) => {
      ...
      if (connection === 'open') {
        ...
        console.log(chalk.bgGreen.black(' CLONE ONLINE ') + chalk.greenBright(` Bot Clone terhubung: +${cleanNum} (${botName})\n`));
        addBotLog('CLONE', `Bot Clone WhatsApp terhubung: +${cleanNum}`);
        ...
        if (statusCode === DisconnectReason.loggedOut) {
          console.log(chalk.yellow(`[CLONE LOGGED OUT] Membersihkan folder sesi untuk +${cleanNum}...`));
          activeCloneSessions.delete(cleanNum);
          ...
          console.log(chalk.yellow(`[CLONE 440 CONFLICT] Sesi +${cleanNum} mengalami konflik (sudah berjalan di proses lain). Menghentikan loop reconnect.`));
          activeCloneSessions.delete(cleanNum);
        } else {
          console.log(chalk.cyan(`[CLONE RECONNECT] Menjaga koneksi socket tetap hidup untuk +${cleanNum}...`));
          setTimeout(() => {
    ...
    if (!cloneSock.authState.creds.registered) {
      ...
      try {
        ...
        addBotLog('PAIRING_ERROR', `Gagal pairing (+${cleanNum}): ${e.message || String(e)} | Stack: ${e.stack || 'No stack'}`);
        ...
        throw new Error(`Gagal meminta kode ke server WhatsApp: ${e.message || 'Error socket WA'}`);
      }
      ...
      addBotLog('PAIRING', `Kode Pairing: ${pair.toUpperCase()} | Nomor User: +${cleanNum}`);
    ...
    throw new Error(`Gagal membuat sesi pairing clone: ${err.message || 'Error socket WA'}`);
  }
}
