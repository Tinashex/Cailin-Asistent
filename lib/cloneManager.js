...
// Use a single pino instance to prevent Vercel dynamic import crash
const logger = pino({ 
  level: 'silent',
  base: null 
});

const CLONES_DB = path.join(process.cwd(), 'data', 'clones.json');
...
export async function loadAllSavedCloneSessions() {
  ...
  for (const num of clones) {
    if (num && num.length >= 9) {
      try {
        console.log(chalk.cyan(`[CLONE AUTO-LOAD] Loading Clone Bot Session: +${num}`));
...
export async function createCloneSession(phoneNumber, forceFresh = false) {
  ...
  // Close existing socket if not registered or force refresh
  ...
  try {
    ...
    cloneSock.ev.on('connection.update', async (update) => {
      ...
      if (connection === 'open') {
        ...
        console.log(chalk.bgGreen.black(' CLONE ONLINE ') + chalk.greenBright(` Clone Bot connected: +${cleanNum} (${botName})\n`));
        addBotLog('CLONE', `Clone WhatsApp Bot connected: +${cleanNum}`);
        ...
        if (statusCode === DisconnectReason.loggedOut) {
          console.log(chalk.yellow(`[CLONE LOGGED OUT] Cleaning session folder for +${cleanNum}...`));
          activeCloneSessions.delete(cleanNum);
          ...
          console.log(chalk.yellow(`[CLONE 440 CONFLICT] Session +${cleanNum} has a conflict (already running in another process). Stopping reconnect loop.`));
          activeCloneSessions.delete(cleanNum);
        } else {
    ...
    if (!cloneSock.authState.creds.registered) {
      ...
      try {
        ...
        addBotLog('PAIRING_ERROR', `Pairing failed (+${cleanNum}): ${e.message || String(e)} | Stack: ${e.stack || 'No stack'}`);
        ...
        throw new Error(`Failed to request code from WhatsApp server: ${e.message || 'WA socket error'}`);
      }
      ...
      addBotLog('PAIRING', `Pairing Code: ${pair.toUpperCase()} | User Number: +${cleanNum}`);
    ...
    console.error(chalk.red('[CLONE ERROR FULL] Failed creating clone session:'), fullErr);
    addBotLog('PAIRING_ERROR', `Clone Error (+${phoneNumber}): ${err.message || String(err)}`);
    throw new Error(`Failed to create clone pairing session: ${err.message || 'WA socket error'}`);
  }
            }
